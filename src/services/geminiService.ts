...
// Find cafes by city
export const findCafesByCity = async (city: string): Promise<Cafe[]> => {
  try {
...
export const generateCafeDescription = async (name: string, location: string) => {
  try {
    const model = genAI.getGenerativeModel({
      model: "gemini-2.0-flash",
    });
    ...
    const result = await model.generateContent(
      `Write a short, inviting Arabic description for a coffee shop named "${name}" in "${location}", under 20 words.`
    );
